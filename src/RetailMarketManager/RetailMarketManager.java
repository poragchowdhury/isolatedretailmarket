...
//        ob.unitcost = (double) Math.round(ob.unitcost * 100) / 100;
        ...
        System.out.println(ob.agentPool.get(0).name + ob.timeslot + "\t\t" + ob.custSubs[0] + "\t" + ob.utility[0] + "\t" + ob.agentPool.get(0).unitcost + "\t" + ob.cost[0] + "\t" + tariff0 + "\t" + ob.money[0] + "\t" + ob.agentPool.get(0).profit);
...
//        pwOutput.println(ob.custSubs[1] + "," + ob.utility[1] + "," + ob.money[1] + "," + ob.cost[1] + "," + ob.agentPool.get(1).profit + "," + ob.agentPool.get(1).tariffPrice + "," + ob.unitcost);
    ...
    public void printAvgRevenues(CaseStudy cs, int iagent, int kagent, PrintWriter pwOutputAvg) {
        ...
        try {
            pwOutputAvg.println("Inertia," + Configuration.INERTIA + ",Rationality," + Configuration.RATIONALITY + "," + cs.pool1.get(iagent).name + "," + payOffVals[0] + "," + cs.pool2.get(kagent).name + "," + payOffVals[1] + ",Error1," + payOffVals[2] + ",Error2," + payOffVals[3]);
...
//        Agent test = new Test();
        Agent alwaysS = new AlwaysIncrease();
