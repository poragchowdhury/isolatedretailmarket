...
public class DQAgentMDP implements MDP<DQAgentState, Integer, DiscreteSpace> {
    ...
    public static DQAgent trainDQAgentNoSaving(DQAgent preTrainedAgent, Agent... opponentPool) {
        return trainDQAgentNoSaving(preTrainedAgent, Arrays.asList(opponentPool));
    }

    public static DQAgent trainDQAgentNoSaving(DQAgent preTrainedAgent, List<Agent> opponentPool) {
        DQAgentMDP mdp = new DQAgentMDP(opponentPool);
        QLearningDiscreteDense<DQAgentState> dql;
        try {
            DataManager manager = new DataManager(true);

            if (preTrainedAgent == null) {
                ...
                IDQN sourceQLNet = preTrainedAgent.pol.getNeuralNet();
                dql = new QLearningDiscreteDense<>(mdp, sourceQLNet, QLConfig, manager);
            }
